...
test('findAndDuplicateCanvas', async (t: tape.Test) => {
  ...
  const duplicatedCanvas = await db.transaction(async (trx: Knex.Transaction) => {
    return findAndDuplicateCanvas(canvas.id, canvas.designId, trx);
  });

  if (!duplicatedCanvas) { throw new Error('Duplicated canvas was not created!'); }

  if (!duplicatedCanvas.componentId) { throw new Error('Canvas has no component!'); }
  const duplicatedComponent = await findComponent(duplicatedCanvas.componentId);
  if (!duplicatedComponent) {
    throw new Error(`Component ${duplicatedCanvas.componentId} does not exist!`);
  }
