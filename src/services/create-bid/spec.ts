...
test("createBid without assignee", async (t: Test) => {
  const now = new Date();
  sandbox().useFakeTimers(now);
  const {
    createCollaboratorStub,
  ...
  t.deepEqual(
    createDesignEventStub.args,
    [],
    "does not call DesignEventsDAO.create"
  );

  t.deepEqual(
    createCollaboratorStub.args,
    [],
    "does not call CollaboratorsDAO.create"
  );

  t.deepEqual(notificationStub.args, [], "does not call Notification service");
});
