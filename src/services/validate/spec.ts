...
test('validateProperties throws an error if properties are missing', async (t: tape.Test) => {
  const testCase = (): void =>
    validateProperties({ foo: 'bar', buz: false }, 'foo', 'buz', 'bar', 'baz');
  t.throws(testCase, InvalidDataError, 'throws correct error constructor');
  t.throws(
    testCase,
...
test('validateProperties throws an error if called on falsy type-cast input', async (t: tape.Test) => {
  const testCase = (): void =>
    validateProperties(null as any, 'foo', 'buz', 'bar', 'baz');
  t.throws(testCase, InvalidDataError, 'throws correct error constructor');
  t.throws(
    testCase,
...
test('validateProperties supports objects that do not inherit from Object.prototype', async (t: tape.Test) => {
  t.doesNotThrow(() => {
...
test('validatePropertiesFormatted throws an error if properties are missing', async (t: tape.Test) => {
  const testCase = (): void =>
    validatePropertiesFormatted(
      { buz: false, foo: 'bar' },
      {
...
test('validateValues throws an error if properties are missing', async (t: tape.Test) => {
  const testCase = (): void =>
    validateValues({
      baz: undefined,
      buz: null,
