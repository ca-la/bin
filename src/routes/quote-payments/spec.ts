...
test('/quote-payments POST generates quotes, payment method, invoice, lineItems, and charges',
async (t: Test) => {
...
test('/quote-payments POST does not generate quotes, payment method, invoice, lineItems on failure',
async (t: Test) => {
  const { user, session } = await createUser();
...
test('/quote-payments?isFinanced=true POST generates quotes, invoice, lineItems',
async (t: Test) => {
  const { user, session } = await createUser();
  sandbox().stub(EmailService, 'enqueueSend').resolves();
  sandbox().stub(SlackService, 'enqueueSend').resolves();
...
test('/quote-payments?isFinanced=true POST does not generate quotes,' +
