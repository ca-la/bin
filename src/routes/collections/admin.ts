...
export function* commitCostInputs(this: Koa.Application.Context): AsyncIterableIterator<any> {
  ...
  NotificationsService.immediatelySendFullyCostedCollection(collectionId, userId);

  this.status = 204;
}

export function* createPartnerPairing(this: Koa.Application.Context): AsyncIterableIterator<any> {
  const { collectionId } = this.params;
  const { userId } = this.state;
