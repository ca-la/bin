...
test('POST /shopify-webhooks/orders-create calls the correct method', t => {
  sandbox()
    .stub(UserAttributesService, 'recordPurchase')
    .returns(Promise.resolve());

  return post('/shopify-webhooks/orders-create', { body: examplePayload }).then(
    ([response]) => {
...
test('POST /shopify-webhooks/orders-create warns if user id is missing', t => {
  ...
  return post('/shopify-webhooks/orders-create', { body: payload }).then(
    ([response]) => {
      t.equal(response.status, 200);
