...
import {
  deleteById as deleteAnnotation
} from '../../components/product-design-canvas-annotations/dao';
...
test('findDesignByAnnotationId', async (t: tape.Test) => {
  ...
  t.equal(designOne.id, design.id, 'Returns the design that is a parent to the annotation');

  await deleteAnnotation(annotationOne.id);
  const designTwo = await findDesignByAnnotationId(annotationOne.id);
  t.equal(designTwo, null, 'Returns null if a resource in the chain was deleted');
});
