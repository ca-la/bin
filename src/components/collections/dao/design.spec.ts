...
test("addDesign, moveDesigns, and removeDesigns", async (t: Test) => {
  ...
  t.deepEqual(
    designs.map((design: ProductDesign) => design.id),
    [d2.id, d1.id]
  );

  const designs2 = await findByCollectionId(c2.id);
  t.deepEqual(
    designs2.map((design: ProductDesign) => design.id),
    [d3.id]
  );
  ...
  t.deepEqual(
    movedDesigns.map((design: ProductDesign) => design.id),
    [d2.id]
  );
  ...
  t.deepEqual(
    movedDesigns3.map((design: ProductDesign) => design.id),
    [d3.id, d1.id]
  );
  ...
  t.deepEqual(
    removedDesigns.map((design: ProductDesign) => design.id),
    [d2.id]
  );
