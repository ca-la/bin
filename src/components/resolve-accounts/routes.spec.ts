...
test(
  'GET /resolve-accounts?userId= returns all accounts for that user',
  async (t: tape.Test) => {
    ...
    const [badResponse] = await API.get(
      `/resolve-accounts?userId=${uuid.v4()}`,
      { headers: API.authHeader(session.id) });
    t.equal(badResponse.status, 403, 'GET returns 403 where user is not autherized');
    const [badQueryResponse] = await API.get(
      '/resolve-accounts',
      { headers: API.authHeader(session.id) });
...
test(
  'POST /resolve-accounts creates a new account with the user and resolve customer id',
  async (t: tape.Test) => {
