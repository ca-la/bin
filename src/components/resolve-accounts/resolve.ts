...
export async function getResolveAccountData(account: ResolveAccount): Promise<ResolveAccountData> {
  ...
  const data = await response.json();
  const unpaidPayLaterInvoiceAmounts: Invoice[] = await InvoicesDAO
    .findByUserAndUnpaid(account.userId);

  const totalUntrackedAmountCents = unpaidPayLaterInvoiceAmounts
    .reduce((acc: number, res: Invoice) => acc + res.totalCents, 0);

  if (data && isRawResolveData(data)) {
    return encodeRawResolveData(data, account, totalUntrackedAmountCents);
  }
  throw new Error(`Could not retrieve Resolve data for customer id ${resolveCustomerId}`);
}

export async function getAllResolveAccountData(
  accounts: ResolveAccount[]
): Promise<ResolveAccountData[]> {
  const withNulls = await Promise.all(accounts
    .map(async (account: ResolveAccount) =>
      await getResolveAccountData(account)));
  return withNulls
    .filter((account: ResolveAccountData | null): account is ResolveAccountData =>
      account !== null);
}
