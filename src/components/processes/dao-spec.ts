...
test('Processes DAO supports finding all processes', async (t: tape.Test) => {
  const { process: p1 } = await generateProcess({ componentType: ComponentType.Material });
  const { process: p2 } = await generateProcess({ componentType: ComponentType.Artwork });
  const { process: p3 } = await generateProcess({ componentType: ComponentType.Sketch });
...
test('Processes DAO supports finding by component type', async (t: tape.Test) => {
  const {
    process: p1
  } = await generateProcess({ componentType: ComponentType.Material, ordering: 0 });
  const {
    process: p2
  } = await generateProcess({ componentType: ComponentType.Material, ordering: 1 });
  const {
    process: p3
  } = await generateProcess({ componentType: ComponentType.Material, ordering: 2 });
  const {
    process: p4
  } = await generateProcess({ componentType: ComponentType.Sketch });

  const materialProcesses = await ProcessesDAO.findAllByComponentType(ComponentType.Material);
  t.deepEqual(materialProcesses, [p1, p2, p3], 'Returns the list in order');
  const sketchProcesses = await ProcessesDAO.findAllByComponentType(ComponentType.Sketch);
  t.deepEqual(sketchProcesses, [p4], 'Returns the list');
  const artworkProcesses = await ProcessesDAO.findAllByComponentType(ComponentType.Artwork);
