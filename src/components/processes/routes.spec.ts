...
test("GET /processes returns all processes", async (t: tape.Test) => {
  ...
  const [getResponse, getBody] = await API.get("/processes", {
  ...
  t.equal(
    getResponseTwo.status,
    200,
    "GET with componentType returns a 200 status"
  );
  t.deepEqual(
    getBodyTwo,
    [{ ...p1, createdAt: p1.createdAt.toISOString() }],
  ...
  const [getResponseThree] = await API.get("/processes?componentType=FooBar", {
    headers: API.authHeader(session.id),
  });
  t.equal(
    getResponseThree.status,
    400,
    "GET with invalid componentType returns a 400 status"
  );

  const [getResponseFour] = await API.get("/processes?componentType=FooBar", {
    headers: API.authHeader("i-am-malicious"),
  });
  t.equal(
    getResponseFour.status,
    401,
    "GET with invalid session token returns a 401 status"
  );
});
