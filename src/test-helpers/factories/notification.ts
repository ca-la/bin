...
import ProductDesignCanvasMeasurement from "../../components/product-design-canvas-measurements/domain-object";
...
export default async function generateNotification(
  ...
  switch (options.type) {
    ...
    case NotificationType.APPROVAL_STEP_SUBMISSION_UNSTARTED: {
      const notification = await create({
        ...baseNotification,
        collectionId: collection.id,
        designId: design.id,
