...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('pricing_cost_inputs', (table: Knex.TableBuilder) => {
  ...
  await knex.schema.createTable('pricing_cost_input_processes', (table: Knex.TableBuilder) => {
    table.uuid('id').primary();
    table.timestamp('created_at').notNullable().defaultTo(knex.fn.now());
